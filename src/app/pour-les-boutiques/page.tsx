...
import { BoutiqueDemoCarousel } from '@/components/landing/BoutiqueDemoCarousel'
...
export const metadata: Metadata = {
  title: 'Organisez vos commandes Instagram et WhatsApp — TerangaSpot',
  description: "Vous vendez déjà sur Instagram, TikTok et WhatsApp. TerangaSpot range vos commandes dans un lien unique — sans rien changer à votre façon de vendre.",
}
...
const steps = [
  {
    ...
    text: "Instagram, TikTok, Stories, statut WhatsApp : continuez à publier vos produits exactement comme aujourd'hui. Rien à changer ici.",
  },
  {
    icon: Link2,
    title: 'Ils commandent sur votre lien',
    text: "Un seul lien en bio renvoie vers votre boutique TerangaSpot. Vos clients choisissent et commandent seuls — sans vous écrire pour demander le prix ou si c'est toujours dispo.",
  },
  {
    icon: MessageCircle,
    title: 'La commande arrive organisée sur WhatsApp',
    text: "Produits, variantes, coordonnées : tout y est déjà. Vous confirmez et vous livrez, exactement comme avant — mais sans reconstituer la commande dans une conversation.",
  },
]
...
const features = [
  ...
  },
  {
    ...
    description: "Vos clients commandent seuls sur votre lien. La commande arrive complète — produits, variantes, coordonnées — directement dans votre WhatsApp. Vous n'avez plus qu'à confirmer.",
  },
  {
    icon: Package,
    title: 'Vos « bientôt disponible », sans repost',
    description: "Ajoutez vos produits une fois, gérez votre stock et activez les précommandes pour vos nouveautés — sans avoir à republier une story à chaque fois qu'on vous demande si c'est arrivé.",
  },
  {
    icon: BarChart3,
    title: 'Vos ventes, sans les noter dans un cahier',
    description: "Vues, commandes, revenus, meilleurs produits : tout est déjà calculé dans votre tableau de bord.",
  },
  {
    icon: BookOpen,
    title: 'Des clients qui ne vous suivent pas encore',
    description: "Votre boutique apparaît automatiquement dans l'annuaire TerangaSpot, consulté par des clients partout au Sénégal — une source de clients en plus d'Instagram et TikTok.",
  },
  {
    icon: Search,
    title: 'Trouvable sur Google aussi',
    description: "Votre boutique est indexée sur Google dès sa création — nom, description, catégorie — sans configuration technique de votre part.",
  },
  {
    icon: Smartphone,
...
// On n'a que 5 boutiques actives à ce jour (cf. section "Elles vendent déjà sur TerangaSpot"
// plus haut sur la page, qui sert de vraie preuve sociale en attendant).
const testimonials = [
  { name: 'Fatou', role: 'Boutique mode, Dakar', stars: 5, content: "Mes clients commandent maintenant directement sur mon lien. Je reçois moins d'appels et des commandes beaucoup plus claires." }, // TODO: témoignage fictif
...
// Charge les deux boutiques démo avec toutes les données nécessaires au vrai composant
// BoutiquePageClient (catégories, produits + variantes, avis, bannières) — même logique
...
async function loadDemoShowcase(): Promise<DemoShowcaseEntry[]> {
  ...
  const { data: subs } = await admin
    .from('subscriptions')
    .select('boutique_id, plan')
    .in('boutique_id', boutiques.map(b => b.id))

  const planByBoutique = new Map((subs ?? []).map(s => [s.boutique_id, s.plan]))

  const sorted = boutiques
    .map(b => ({ ...b, plan: (planByBoutique.get(b.id) ?? 'starter') as 'starter' | 'pro' }))
...
// Boutiques réelles et actives (is_demo = false) — la vraie preuve sociale de la page.
// On est encore petits (quelques boutiques), donc on montre de la profondeur (des vraies
// boutiques cliquables) plutôt que d'inventer du volume.
async function loadRealBoutiques(): Promise<RealBoutiquePreview[]> {
  const supabase = createClient()
...
const faqs = [
  { q: 'Est-ce que je vais perdre le contact direct avec mes clients ?', a: "Non. Les commandes arrivent toujours sur votre WhatsApp, exactement comme aujourd'hui. TerangaSpot organise juste ce qui arrive avant — vous gardez toute la conversation avec vos clients." },
  { q: "Je vends déjà bien sur Instagram, pourquoi j'aurais besoin de ça ?", a: "Parce que le problème n'est pas de vendre, c'est de gérer ce que vous vendez déjà. Si vous répondez encore \"c'est combien ?\" en message privé dix fois par jour, ou si vous avez déjà perdu une commande dans vos DM, TerangaSpot règle ça." },
  { q: "Je ne suis pas doué avec la technologie, c'est compliqué à utiliser ?", a: "Non. Si vous savez publier une photo sur Instagram, vous savez utiliser TerangaSpot." },
  { q: "Est-ce que ça va me coûter cher en internet/data pour gérer ma boutique ?", a: "Non, le tableau de bord est très léger, pensé pour fonctionner même avec une petite connexion." },
  { q: 'Et si ça ne marche pas pour moi, je perds mon argent ?', a: "Non — vous pouvez créer votre boutique gratuitement avec TerangaSpot Free et l'utiliser sans limite de temps. Vous ne payez que si vous décidez d'aller plus loin." },
  { q: "Je n'ai pas beaucoup de produits, ça vaut le coup pour une petite boutique ?", a: "Oui. Avec la formule gratuite, vous pouvez déjà publier jusqu'à 10 produits — largement de quoi tester et avoir une vraie boutique professionnelle en ligne." },
  ...
  { q: "Si j'arrête, qu'est-ce qui se passe ?", a: "Rien n'est supprimé. Votre boutique reste en pause jusqu'à réabonnement, et vous pouvez télécharger toutes vos données (commandes, comptabilité) en Excel depuis votre espace commerçant." },
]

export default async function PourLesBoutiquesPage() {
  ...
  const [demoShowcase, realBoutiques] = await Promise.all([
    loadDemoShowcase(),
    loadRealBoutiques(),
  ])

  // Produit réel (issu des boutiques démo, pas inventé) utilisé dans le mockup téléphone
  // du hero — photo, nom, prix et boutique authentiques plutôt qu'un placeholder générique.
  const allDemoProducts = demoShowcase.flatMap(e =>
    e.products.map(p => ({ ...p, boutiqueName: e.boutique.name, boutiqueSlug: e.boutique.slug }))
  )
  ...
  const heroProduct = heroProductSource ? {
    ...
    boutiqueName: heroProductSource.boutiqueName,
    variant: heroVariant,
  } : null
  ...
  const heroOrderMessage = heroProductSource ? buildWhatsAppMessage(
    heroProductSource.boutiqueName,
    [{
      name: heroProductSource.name,
      quantity: 1,
      price: heroProductSource.price,
    ...
    '771234567',
    {
      ...
      paymentMethod: 'Wave',
      dashboardUrl: `${getSiteUrl()}/c/${heroProductSource.boutiqueSlug}/TS-000004`,
    }
  ) : null
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-20 sm:py-28 grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
          <div className="text-center lg:text-left">
            ...
            <p className="text-lg sm:text-xl mb-8" style={{ color: 'rgba(255,255,255,0.65)' }}>
              Vous vendez déjà sur Instagram, TikTok et WhatsApp. TerangaSpot range vos commandes dans un lien unique — vos clients commandent seuls, vous recevez tout, organisé, sur WhatsApp.
            </p>
      ...
      {/* Preuve réelle — logos des boutiques actives qui défilent, pas une grosse section */}
      ...
      {/* Comment ça marche — le parcours Instagram/TikTok → TerangaSpot → WhatsApp.
      ...
      {/* Aperçu de l'interface — démonstration interactive du vrai produit.
      ...
      {demoShowcase.length > 0 && (
        <section className="relative -mt-8 sm:-mt-10 pt-16 sm:pt-20 pb-16 px-4 overflow-hidden" style={{ backgroundColor: '#0A0A0A' }}>
          ...
          <div className="relative max-w-5xl mx-auto">
            ...
            <BoutiqueDemoCarousel entries={demoShowcase} />
          </div>
        </section>
      )}
            ...
            {features.map(f => (
              <div key={f.title} className="bg-white rounded-2xl p-6 border border-gray-100 transition-colors hover:border-orange-200">
                ...
                <p className="text-gray-500 text-sm leading-relaxed">
                  {f.description ?? `${siteHost}/votre-boutique — partagez-le partout : Instagram, Snapchat, WhatsApp, cartes de visite. Vos clients arrivent directement sur votre vitrine, sans DM.`}
                </p>
              </div>
            ))}
          </div>
