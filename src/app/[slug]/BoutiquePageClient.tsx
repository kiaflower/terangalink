...
interface Props {
  ...
  plan?: PlanKey
  banners?: Banner[]
  // Mode utilisé par le mockup animé de /pour-les-boutiques : désactive les effets de
...
export default function BoutiquePageClient({ boutique, categories, products, reviews = [], fromAnnuaire, plan = 'starter', banners = [], previewMode = false, demoHomeHref = '/' }: Props) {
  const { accent, theme, isDark, isVibrant, pageBg, pageText, cardBg, cardBorder, subtleText } = getBoutiqueTheme(boutique)
  // Boucle d'acquisition "Propulsé par TerangaSpot" (Free/Starter) : le lien
  // porte le code de parrainage de la boutique, réutilisant le système de
  // parrainage existant (dashboard/boutique/parrainage) pour récompenser la
  // boutique d'origine si le clic mène à une inscription payante.
  const referralUrl = boutique.referral_code
    ? `https://teranga-spot.com/inscription?ref=${boutique.referral_code}`
    : 'https://teranga-spot.com'
  const [activeCategory, setActiveCategory] = useState<string | null>(null)
  const { cart, setCart, clearCart } = usePersistedCart(boutique.slug)
  ...
  const boutiqueUrl = typeof window !== 'undefined' ? `${window.location.origin}/${boutique.slug}` : `https://teranga-spot.com/${boutique.slug}`

  useEffect(() => {
    function refresh() {
      setOpenStatus(getOpenStatus(boutique.opening_hours))
      setNow(Date.now())
    }
    refresh()
  ...
  useEffect(() => {
    ...
    supabase.from('boutique_stories').select('*').eq('boutique_id', boutique.id)
  ...
  useEffect(() => {
    if (qrOpen && qrRef.current) {
      import('qrcode').then(QRCode => {
        QRCode.toCanvas(qrRef.current!, boutiqueUrl, { width: 200, color: { dark: accent, light: '#ffffff' } })
      })
    }
  }, [qrOpen, boutiqueUrl, accent])
  ...
  function trackEvent(eventType: string, itemId?: string, itemName?: string) {
    ...
    fetch('/api/analytics/track', {
      ...
      body: JSON.stringify({
        boutique_id: boutique.id, event_type: eventType, item_id: itemId, item_name: itemName,
        session_id: getOrCreateSessionId(boutique.slug),
      }),
    }).catch(() => {})
  }
  ...
  async function handleShare() {
    const url = boutiqueUrl
    if (navigator.share) {
      try { await navigator.share({ title: boutique.name, text: `Découvrez ${boutique.name} sur TerangaSpot`, url }) } catch { /* cancelled */ }
    } else {
  ...
  async function applyPromoCode() {
    ...
    try {
      const res = await fetch('/api/promo-codes/validate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ boutique_id: boutique.id, code: promoCodeInput.trim(), subtotal: cartTotal }),
      })
  ...
  async function sendWhatsApp() {
    ...
    try {
      const res = await fetch('/api/orders', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          boutique_id: boutique.id, customer_name: customerName, customer_phone: customerPhone, customer_address: customerAddress,
    ...
    const message = buildWhatsAppMessage(boutique.name, items, cartTotal, customerName, customerPhone, {
      notes,
    ...
    const waUrl = `https://wa.me/${boutique.whatsapp_number.replace(/\D/g, '')}?text=${encodeURIComponent(message)}`
  ...
  function sendAnnouncementWhatsApp() {
    const title = boutique.announcement_title?.trim()
    const message = title
      ? `Bonjour, je souhaite obtenir plus d'infos sur : ${title} - ${boutique.name}`
      : `Bonjour, je souhaite obtenir plus d'infos sur votre annonce - ${boutique.name}`
    const waUrl = `https://wa.me/${boutique.whatsapp_number.replace(/\D/g, '')}?text=${encodeURIComponent(message)}`
  ...
  const navLinks = [
    ...
    ...(boutique.description ? [{ href: '#apropos', label: 'À propos' }] : []),
    ...
    <div className="min-h-screen" style={{ backgroundColor: pageBg, color: pageText }}>
      ...
      {boutique.is_demo && (
        <div className="sticky top-0 z-50 flex items-center justify-between px-4 text-white text-xs font-semibold"
          ...
          <span className="hidden sm:inline">Ceci est une démo TerangaSpot</span>
          <Link href="/inscription" className="hover:underline">Créer ma boutique →</Link>
        </div>
      )}

      {/* Sticky nav */}
      <header className="sticky z-40" style={{ top: boutique.is_demo ? 40 : 0, backgroundColor: pageBg, borderBottom: `1px solid ${cardBorder}` }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <button
              type="button"
              onClick={() => stories.length > 0 && setStoriesViewerOpen(true)}
              aria-label={stories.length > 0 ? 'Voir les stories' : boutique.name}
            ...
            >
              {boutique.logo_url ? (
                ...
                <img src={boutique.logo_url} alt={boutique.name} className="w-9 h-9 rounded-lg object-cover" />
              ) : (
            ...
            <div>
              <p className="font-bold text-sm leading-tight" style={{ color: pageText }}>{boutique.name}</p>
              <p className="text-[10px] uppercase tracking-wider" style={{ color: subtleText }}>{boutique.shop_category}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {boutique.whatsapp_number && (
              <a href={`tel:${boutique.whatsapp_number.replace(/\D/g, '')}`}
      ...
      {mobileNavOpen && (
        <div className="fixed inset-0 z-50 flex flex-col" style={{ backgroundColor: pageBg }}>
          <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: `1px solid ${cardBorder}` }}>
            <span className="font-bold text-lg" style={{ color: pageText }}>{boutique.name}</span>
      ...
      {fromAnnuaire && (
        <div className="border-b border-gray-100 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2">
            <Link href="/boutiques" className="inline-flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-gray-900 transition-colors">
      ...
      {(
          <section id="accueil" className="relative h-64 sm:h-80 overflow-hidden">
            {boutique.cover_url ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={boutique.cover_url} alt={boutique.name} className="w-full h-full object-cover" />
            ) : heroProduct?.image_url ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={heroProduct.image_url} alt={boutique.name} className="w-full h-full object-cover" />
            ) : (
              <div className="w-full h-full" style={{ backgroundColor: accent }} />
            )}
            <div className="absolute inset-0 bg-black/45" />
            {!canUseFeature(plan, 'suppressionBranding') && !previewMode && !boutique.is_demo && (
              <a href={referralUrl} target="_blank" rel="noopener noreferrer"
                ...
                aria-label="Propulsé par TerangaSpot">
            ...
            <div className="absolute inset-0 flex flex-col justify-end px-6 pb-8">
              {boutique.shop_category && (
                <p className="text-xs font-bold uppercase tracking-widest text-white/70 mb-1">{boutique.shop_category}</p>
              )}
              ...
              {boutique.description && (
                <p className="text-white/75 text-sm leading-relaxed max-w-lg">{boutique.description}</p>
              )}
              {openStatus && (
                <div className="mt-3">
                  {openStatus.isOpen ? (
      ...
      {canUseFeature(plan, 'annonce') && boutique.announcement_enabled && boutique.announcement_image_url && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-6">
          ...
          >
            <Image
              src={boutique.announcement_image_url}
              alt={boutique.announcement_title?.trim() || `Annonce spéciale ${boutique.name}`}
              fill
      ...
      {/* Products */}
      <section id="produits" className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        <h2 className="text-xl sm:text-2xl font-bold text-center mb-8" style={{ color: pageText }}>Articles</h2>
        {filteredProducts.length === 0 ? (
          <div className="text-center py-20 text-gray-400">Aucun produit disponible</div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-5">
            {filteredProducts.map(product => (
              <div key={product.id} className="rounded-2xl overflow-hidden hover:shadow-md transition-shadow group" style={{ backgroundColor: cardBg, border: `1px solid ${cardBorder}` }}>
                <Link
                  href={`/${boutique.slug}/produit/${product.slug}`}
                  onClick={e => {
      ...
      {/* Delivery info */}
      {boutique.show_delivery_info && boutique.delivery_info && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
          <div className="rounded-2xl p-5 sm:p-6 flex items-center gap-4" style={{ backgroundColor: cardBg, border: `1px solid ${cardBorder}` }}>
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: subtleText }}>{boutique.delivery_info}</p>
            </div>
          </div>
        </section>
      )}
      ...
      <footer id="contact" className="mt-12" style={{ backgroundColor: cardBg, borderTop: `1px solid ${cardBorder}` }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 grid grid-cols-1 sm:grid-cols-3 gap-8">
          <div>
            <div className="flex items-center gap-2 mb-3">
              {boutique.logo_url ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={boutique.logo_url} alt="" loading="lazy" decoding="async" className="w-8 h-8 rounded-lg object-cover" />
              ) : (
            ...
            <p className="text-xs mb-4" style={{ color: subtleText }}>Des produits de qualité, un service de confiance.</p>
            <div className="flex gap-2">
              {boutique.facebook_url && (
                <a href={boutique.facebook_url} target="_blank" rel="noopener noreferrer"
                  className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors"
                  style={{ border: `1px solid ${cardBorder}`, color: pageText }}>
                  <FacebookIcon className="w-3.5 h-3.5" />
                </a>
              )}
              {boutique.instagram_url && (
                <a href={boutique.instagram_url} target="_blank" rel="noopener noreferrer"
                  className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors"
                  style={{ border: `1px solid ${cardBorder}`, color: pageText }}>
              ...
              {boutique.tiktok_url && (
                <a href={boutique.tiktok_url} target="_blank" rel="noopener noreferrer"
                  className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors"
                  style={{ border: `1px solid ${cardBorder}`, color: pageText }}>
                  <TikTokIcon className="w-3.5 h-3.5" />
                </a>
              )}
              {boutique.snapchat_url && (
                <a href={boutique.snapchat_url} target="_blank" rel="noopener noreferrer"
                  className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors"
                  style={{ border: `1px solid ${cardBorder}`, color: pageText }}>
          ...
          <div>
            ...
            <div className="space-y-2">
              {boutique.whatsapp_number && (
                <a href={`https://wa.me/${boutique.whatsapp_number.replace(/\D/g, '')}`}
              ...
              {boutique.phone && boutique.phone !== boutique.whatsapp_number && (
                <a href={`tel:${boutique.phone}`} className="text-xs flex items-center gap-1.5" style={{ color: subtleText }}>
                  <Phone className="w-3.5 h-3.5 flex-shrink-0" />{boutique.phone}
                </a>
              )}
              <a
                href={`https://www.google.com/maps/search/${encodeURIComponent((boutique.city ?? 'Dakar') + ', Sénégal')}`}
                target="_blank" rel="noopener noreferrer"
                className="text-xs flex items-center gap-1.5" style={{ color: subtleText }}>
                <svg className="w-3.5 h-3.5 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/>
                </svg>
                {boutique.city ?? 'Dakar'}, Sénégal
              </a>
            </div>
          </div>
        </div>
        ...
        <div className="py-4 text-center" style={{ borderTop: `1px solid ${cardBorder}` }}>
          <p className="text-xs" style={{ color: subtleText }}>
            © 2026 {boutique.name}. Tous droits réservés.
          </p>
        </div>

        {!canUseFeature(plan, 'suppressionBranding') && !boutique.is_demo && (
      ...
      {announcementZoomOpen && boutique.announcement_image_url && (
        <ImageLightbox
          images={[boutique.announcement_image_url]}
          index={0}
          alt={boutique.announcement_title?.trim() || `Annonce spéciale ${boutique.name}`}
      ...
      {qrOpen && canUseFeature(plan, 'qrCode') && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-xs p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-gray-900">QR Code — {boutique.name}</h3>
      ...
      {storiesViewerOpen && stories.length > 0 && (
        <StoryViewer
          ...
          boutiqueSlug={boutique.slug}
          boutiqueName={boutique.name}
          boutiqueLogoUrl={boutique.logo_url}
