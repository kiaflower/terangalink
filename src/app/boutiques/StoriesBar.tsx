...
export interface StoryBoutique {
  id: string
  slug: string
  name: string
...
interface Props {
  boutiques: StoryBoutique[]
}

export function StoriesBar({ boutiques }: Props) {
  const [viewedIds, setViewedIds] = useState<Set<string>>(() => new Set())
  const [openBoutique, setOpenBoutique] = useState<StoryBoutique | null>(null)
  const [viewerStories, setViewerStories] = useState<BoutiqueStory[]>([])
  ...
  async function openStories(b: StoryBoutique) {
    const supabase = createClient()
    const { data } = await supabase.from('boutique_stories').select('*').eq('boutique_id', b.id)
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 pt-6">
      ...
      {openBoutique && viewerStories.length > 0 && (
        <StoryViewer
          stories={viewerStories}
          boutiqueSlug={openBoutique.slug}
          boutiqueName={openBoutique.name}
          boutiqueLogoUrl={openBoutique.logo_url}
          onClose={() => setOpenBoutique(null)}
        />
      )}
    </div>
  )
