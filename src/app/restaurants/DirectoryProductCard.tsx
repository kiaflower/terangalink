...
export function DirectoryProductCard({ product: p, restaurant: b }: Props) {
  function trackClick() {
    fetch('/api/analytics/track', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ restaurant_id: b.id, event_type: 'directory_click', item_id: p.id, item_name: p.name }),
    }).catch(() => {})
  }
  ...
  return (
    <Link
      href={`/${b.slug}/plat/${p.slug}?from=annuaire`}
      onClick={trackClick}
