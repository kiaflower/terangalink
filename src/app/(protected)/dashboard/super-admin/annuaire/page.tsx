...
interface Restaurant {
  id: string; name: string; slug: string; cuisine_type: string | null; city: string | null
  is_active: boolean; is_verified: boolean
}

interface RestaurantStats extends Restaurant {
...
export default function AnnuairePage() {
  ...
  const load = useCallback(async () => {
    ...
    const [
      ...
      { count: viewsToday },
      { count: viewsWeek },
      { data: viewsPerRestaurant },
    ] = await Promise.all([
      supabase.from('restaurants').select('id, name, slug, cuisine_type, city, is_active, is_verified').order('name').limit(200),
      supabase.from('analytics_events').select('*', { count: 'exact', head: true }).eq('event_type', 'restaurant_view'),
      supabase.from('analytics_events').select('*', { count: 'exact', head: true }).eq('event_type', 'restaurant_view').gte('created_at', today.toISOString()),
      supabase.from('analytics_events').select('*', { count: 'exact', head: true }).eq('event_type', 'restaurant_view').gte('created_at', week.toISOString()),
      supabase.from('analytics_events').select('restaurant_id').eq('event_type', 'restaurant_view'),
    ])
    ...
    ;(viewsPerRestaurant ?? []).forEach((e: { restaurant_id: string; created_at?: string }) => {
      if (e.restaurant_id) {
        viewMap[e.restaurant_id] = (viewMap[e.restaurant_id] ?? 0) + 1
        if ((e as { created_at?: string }).created_at && (e as { created_at?: string }).created_at! >= todayStr) {
          viewMapToday[e.restaurant_id] = (viewMapToday[e.restaurant_id] ?? 0) + 1
        }
      }
    })

    // We can't easily get created_at per restaurant above because we didn't select it. Let's just use total.
    const restaurantList = (b ?? []) as Restaurant[]
    const withStats: RestaurantStats[] = restaurantList.map(bq => ({
      ...bq,
    ...
    setStats({
      ...
      active: restaurantList.filter(x => x.is_active).length,
      verified: restaurantList.filter(x => x.is_verified).length,
  ...
  async function toggle(id: string, field: 'is_active' | 'is_verified', current: boolean) {
    ...
    setRestaurants(prev => prev.map(b => b.id === id ? { ...b, [field]: !current } : b))
    setStats(prev => ({
      ...prev,
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        <StatCard label="Restaurants total" value={stats.total} icon={Store} color="#F97316" />
        ...
        <StatCard label="Vues restaurants (total)" value={stats.total_views.toLocaleString('fr-SN')} icon={Eye} color="#D97706" />
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        ...
        {loading ? (
          ...
          <p className="text-center text-gray-500 py-20 text-sm">Aucun restaurant</p>
        ) : (
            ...
            {filtered.map(b => (
              <div key={b.id} className="p-4 sm:p-0 sm:grid sm:grid-cols-12 sm:gap-3 sm:px-5 sm:py-4 items-center gap-3 flex flex-col sm:flex-row">
                <div className="w-full sm:col-span-4 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 truncate">{b.cuisine_type}{b.city ? ` · ${b.city}` : ''}</p>
                </div>
