...
import { BoutiqueRowActions } from './BoutiqueRowActions'
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-gray-200 text-sm shadow-sm" style={{ backgroundColor: bg }}>
      ...
      <div className="px-3 py-2.5">
        <p className="font-bold text-sm truncate" style={{ color: text }}>{name || 'Nom boutique'}</p>
...
function CreateBoutiqueModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  const [step, setStep] = useState(1)
  const [form, setForm] = useState({
    // Boutique info
    name: '', slug: '', description: '', shop_category: '', city: 'Dakar',
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setResult({ email: form.admin_email, password: form.admin_password, slug: data.boutique.slug, name: data.boutique.name })
      setLoading(false)
      onCreated()
    } catch {
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl w-full max-w-5xl max-h-[95vh] flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: '1px solid #F3F4F6' }}>
          <div>
            <h2 className="text-lg font-bold text-gray-900">Créer une boutique</h2>
            <p className="text-xs text-gray-400 mt-0.5">Onboardez une boutique complète en quelques étapes</p>
          </div>
                    ...
                    <div className="bg-gray-50 rounded-2xl p-5 space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <select value={form.shop_category} onChange={e => set('shop_category', e.target.value)} className={iCls}>
                        ...
                        <div className="sm:col-span-2">
                          ...
                          <textarea rows={2} value={form.description} onChange={e => set('description', e.target.value)}
                            placeholder="Présentation courte de la boutique…"
                    ...
                    <div className="bg-gray-50 rounded-2xl p-5 space-y-4">
                      ...
                      {form.plan !== 'pro' && (
                        <p className="text-xs text-gray-400">
                          Plan Starter sélectionné : la boutique gardera les couleurs et le thème par défaut de TerangaSpot.
                        </p>
                      )}
                ...
                {step === 3 && (
                  <div className="bg-gray-50 rounded-2xl p-5 space-y-4">
                    ...
                    {[
                      ...
                      { key: 'instagram_url' as const, label: 'Instagram', placeholder: 'https://instagram.com/maboutique' },
                      { key: 'tiktok_url' as const, label: 'TikTok', placeholder: 'https://tiktok.com/@maboutique' },
                      { key: 'snapchat_url' as const, label: 'Snapchat', placeholder: 'https://snapchat.com/add/maboutique' },
                ...
                {step === 4 && (
                  <div className="bg-gray-50 rounded-2xl p-5 space-y-4">
                    ...
                    <div>
                      ...
                      <input type="email" required value={form.admin_email} onChange={e => set('admin_email', e.target.value)} placeholder="admin@boutique.com" className={iCls} />
                    </div>
                    <div>
              ...
              <div className="hidden lg:block p-6 border-l border-gray-100 bg-gray-50/50">
                ...
                {form.name && (
                  <div className="mt-4 space-y-1.5">
                    ...
                    {form.shop_category && <p className="text-xs text-gray-500">🏪 {form.shop_category}</p>}
...
export default function SuperAdminBoutiquesPage() {
  ...
  const fetchBoutiques = useCallback(async () => {
    const { data } = await supabase
      .from('boutiques')
      .select('id, name, slug, shop_category, city, is_active, primary_color, logo_url, created_at, subscriptions(plan)')
      .order('created_at', { ascending: false })
      .limit(200)
    const rows = (data ?? []) as unknown as Array<Omit<Boutique, 'plan' | 'lastReportSentAt'> & { subscriptions: { plan: string } | { plan: string }[] | null }>

    const { data: reports } = await supabase
      .from('weekly_reports')
      .select('boutique_id, sent_at')
      .order('sent_at', { ascending: false })
      .limit(1000)
    const lastReportByBoutique = new Map<string, string>()
    for (const r of (reports ?? []) as Array<{ boutique_id: string; sent_at: string }>) {
      if (!lastReportByBoutique.has(r.boutique_id)) lastReportByBoutique.set(r.boutique_id, r.sent_at)
    }

    setBoutiques(rows.map(b => {
      const sub = Array.isArray(b.subscriptions) ? b.subscriptions[0] : b.subscriptions
      return { ...b, plan: (sub?.plan as Boutique['plan']) ?? null, lastReportSentAt: lastReportByBoutique.get(b.id) ?? null }
    }))
    setLoading(false)
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('boutiques-list')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'boutiques' }, fetchBoutiques)
      .subscribe()
  ...
  const planCounts = boutiques.reduce((acc, b) => {
    ...
    <div>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">{boutiques.length} boutique{boutiques.length !== 1 ? 's' : ''} enregistrée{boutiques.length !== 1 ? 's' : ''}</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <button onClick={fetchBoutiques} className="p-2.5 rounded-xl border border-gray-200 hover:bg-gray-50 transition-colors">
            ...
            <div key={b.id} className={`p-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 ${i < filtered.length - 1 ? 'border-b border-gray-100' : ''}`}>
              <div className="flex items-center gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5 truncate">
                    {[b.shop_category, b.city, `/${b.slug}`].filter(Boolean).join(' · ')}
                  </p>
                </div>
              </div>
              {/* Actions */}
              <div className="flex flex-wrap items-center gap-2 sm:gap-3 sm:shrink-0">
                ...
                <BoutiqueRowActions
                  ...
                  onToggled={(id, newIsActive) => setBoutiques(prev => prev.map(x => x.id === id ? { ...x, is_active: newIsActive } : x))}
                  lastReportSentAt={b.lastReportSentAt}
                  onReportSent={(id, sentAt) => setBoutiques(prev => prev.map(x => x.id === id ? { ...x, lastReportSentAt: sentAt } : x))}
                />
              </div>
            </div>
          ))
