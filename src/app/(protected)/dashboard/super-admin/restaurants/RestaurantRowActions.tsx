...
export function RestaurantRowActions({
  ...
  async function sendWeeklyReport() {
    if (sentThisWeek && !confirm(`Un rapport a déjà été envoyé à "${restaurantName}" le ${new Date(lastReportSentAt!).toLocaleDateString('fr-SN')}. Préparer un nouveau message quand même ?`)) return
    setLoading('report')
    ...
    try {
      const res = await fetch(`/api/super-admin/restaurants/${restaurantId}/send-weekly-report`, { method: 'POST' })
      ...
      if (res.ok) {
        ...
        onReportSent?.(restaurantId, data.sentAt ?? new Date().toISOString())
      } else {
        waWindow?.close()
  ...
  async function toggleActive() {
    setLoading('toggle')
    try {
      const res = await fetch('/api/super-admin/toggle-restaurant', {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        onToggled?.(restaurantId, data.is_active ?? !isActive)
      } else {
  ...
  async function resetStats() {
    if (!confirm(`Réinitialiser toutes les statistiques de "${restaurantName}" ? Ceci supprime définitivement toutes ses commandes et ses vues (revenus, nombre de commandes et vues repartent à 0). Cette action est irréversible.`)) return
    setLoading('reset-stats')
    try {
      const res = await fetch('/api/super-admin/reset-restaurant-stats', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ restaurant_id: restaurantId }),
      })
  ...
  async function remove() {
    if (!confirm('Supprimer définitivement ce restaurant et toutes ses données ? Cette action est irréversible.')) return
    setLoading('delete')
    try {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      <Link
        href={`/dashboard/super-admin/restaurants/${restaurantId}/edit`}
        aria-label="Modifier"
        title="Modifier"
        className={`${iconBtnClass} text-gray-500 border-gray-200 hover:border-brand-orange/30 hover:text-brand-orange`}
      >
