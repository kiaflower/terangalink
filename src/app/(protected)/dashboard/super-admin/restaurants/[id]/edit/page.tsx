...
function RestaurantPreview({ name, color, theme, logoUrl, coverUrl }: { name: string; color: string; theme: string; logoUrl?: string; coverUrl?: string }) {
  const { accent, pageBg: bg, pageText: text, cardBg } = getRestaurantTheme({ primary_color: color, theme })
...
export default function EditRestaurantPage() {
  ...
  const [form, setForm] = useState({
    name: '', slug: '', city: '', cuisine_type: '', description: '',
  ...
  async function loadAdmins() {
    ...
    const [{ data: profiles }, { data: sub }] = await Promise.all([
      supabase.from('profiles').select('id, full_name, email, created_at').eq('restaurant_id', id).eq('role', 'restaurant_admin').eq('is_active', true),
      supabase.from('subscriptions').select('id, plan, status, ends_at').eq('restaurant_id', id).limit(1).single(),
    ])
    ...
    const res = await fetch('/api/super-admin/update-subscription', {
      ...
      body: JSON.stringify(
        subForm.id
          ...
          : { restaurant_id: id, plan: subForm.plan, status: subForm.status, ends_at: subForm.ends_at || undefined }
      ),
    })
    setSubSaving(false)
    if (!res.ok) {
  ...
  async function addAdmin() {
    ...
    const res = await fetch('/api/super-admin/add-restaurant-admin', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ restaurant_id: id, full_name: addAdminForm.full_name, email: addAdminForm.email, password: pwd }),
    })
    ...
    supabase.from('restaurants').select('*').eq('id', id).single().then(({ data }) => {
      if (data) {
        ...
        setForm({
          ...
          cuisine_type: (d.cuisine_type as string) ?? '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/super-admin/edit-restaurant', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    setSuccess(true)
    setSaving(false)
    setTimeout(() => router.push('/dashboard/super-admin/restaurants'), 1200)
    } catch {
  ...
  return (
    <div className="max-w-5xl">
      <Link href="/dashboard/super-admin/restaurants" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-brand-orange transition-colors mb-6">
      ...
      {success && <div className="bg-green-50 border border-green-200 text-green-700 text-sm rounded-xl px-4 py-3 mb-6">Restaurant mise à jour avec succès.</div>}
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select value={form.cuisine_type} onChange={e => setForm(f => ({ ...f, cuisine_type: e.target.value }))} className={inputClass}>
          ...
          {/* Color & theme — Pro only, Starter keeps TerangaLink's default colors */}
          <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-4">
            ...
            {plan !== 'pro' && (
              <p className="text-xs text-gray-400 -mt-2">
                Ce restaurant est en plan Starter : elle conserve les couleurs et le thème par défaut de TerangaLink. Passez-la en Pro pour personnaliser.
              </p>
            )}
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6">
          ...
          {!subForm.id && (
            <p className="text-xs text-amber-600 mt-3">
              Ce restaurant n&apos;a pas encore d&apos;abonnement enregistré — la sauvegarde en créera un.
            </p>
          )}
          {subForm.plan !== 'pro' && (
            <p className="text-xs text-gray-400 mt-3">
              En plan {subForm.plan === 'free' ? 'Free' : 'Starter'}, le restaurant garde automatiquement les couleurs et le thème par défaut de TerangaLink.
            </p>
          )}
        </div>
      </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6">
          ...
          {adminsLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-400">Aucun admin pour ce restaurant</p>
            </div>
          ) : (
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <input type="email" value={addAdminForm.email}
                      ...
                      placeholder="admin@restaurant.com" className={inputClass} />
                  </div>
                  {admins.length === 0 ? (
                    <div>
