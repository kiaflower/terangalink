...
export default function PromotionsPage() {
  ...
  const load = useCallback(async (bid: string) => {
    const { data } = await supabase.from('promo_codes').select('*').eq('boutique_id', bid).order('created_at', { ascending: false })
    setCodes(data ?? [])
  }, [supabase])
  ...
  async function saveCode() {
    if (!boutiqueId || !form.code || !form.discount_value) return
    setSaving(true)
    const payload = {
    ...
    if (editingId) {
      ...
      await supabase.from('promo_codes').insert({ boutique_id: boutiqueId, ...payload })
    }
  ...
  async function toggleCode(id: string, is_active: boolean) {
    ...
    if (boutiqueId) await load(boutiqueId)
  }
