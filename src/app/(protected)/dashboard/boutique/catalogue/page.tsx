...
import type { ProductCategory, ProductWithVariants } from '@/lib/types'
...
export default function CataloguePage() {
  ...
  // Un produit qui avait des variantes à l'ouverture de l'éditeur et qui les
  ...
  const load = useCallback(async (bid: string) => {
    const [{ data: cats }, { data: prods }] = await Promise.all([
      supabase.from('product_categories').select('*').eq('boutique_id', bid).order('position'),
      supabase.from('products').select('*, variants:product_variants(*)').eq('boutique_id', bid).order('is_pinned', { ascending: false }).order('position'),
    ])
    setCategories(cats ?? [])
    setProducts(prods ?? [])
  }, [supabase])
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      if (bid) {
        setBoutiqueId(bid)
        load(bid)
        const { data: sub } = await supabase.from('subscriptions').select('plan').eq('boutique_id', bid).single()
        if (sub?.plan === 'pro' || sub?.plan === 'free') setPlan(sub.plan)
        const { data: b } = await supabase.from('boutiques')
  ...
  async function uploadAnnouncementImage(file: File) {
    ...
    const path = `${boutiqueId}/announcement-${Date.now()}-${cropped.name}`
    const { error } = await supabase.storage.from('boutique-images').upload(path, cropped, { upsert: false })
    if (!error) {
      const { data } = supabase.storage.from('boutique-images').getPublicUrl(path)
  ...
  async function handleVideoUpload(file: File) {
    ...
    const path = `${boutiqueId}/${Date.now()}-${Math.random().toString(36).slice(2)}-${file.name}`
  ...
  async function handleImageUpload(files: FileList | null) {
    if (!files || !files.length || !boutiqueId) return
    setUploading(true)
    const uploaded: string[] = []
    for (const file of Array.from(files)) {
      if (form.images.length + uploaded.length >= maxImages) break
      const compressed = await fileToCompressedBlob(file, 1600)
      const path = `${boutiqueId}/${Date.now()}-${Math.random().toString(36).slice(2)}-${compressed.name}`
  ...
  // active. Si le produit avait déjà des variantes à l'ouverture de
  ...
  // principale du produit. Même convention que les photos produit (compression
  // client-side + bucket product-images).
  async function uploadVariantOptionImage(variantIndex: number, option: string, file: File) {
    if (!boutiqueId) return
    const compressed = await fileToCompressedBlob(file, 1600)
    const path = `${boutiqueId}/variant-${Date.now()}-${Math.random().toString(36).slice(2)}-${compressed.name}`
    const { error } = await supabase.storage.from('product-images').upload(path, compressed, { upsert: false })
    if (error) return
    const { data } = supabase.storage.from('product-images').getPublicUrl(path)
  ...
  async function saveProduct() {
    ...
    const payload = {
      ...
      is_pinned: canUseFeature(plan, 'epinglageProduits') && form.is_pinned,
  ...
  async function deleteProduct(id: string) {
    if (!boutiqueId || !confirm('Supprimer ce produit ?')) return
    await supabase.from('products').delete().eq('id', id)
    await load(boutiqueId)
  }

  async function addCategory() {
    if (!boutiqueId || !catName.trim()) return
    setAddingCat(true)
    await supabase.from('product_categories').insert({ boutique_id: boutiqueId, name: catName.trim(), position: categories.length })
    await load(boutiqueId)
    setCatName('')
    setAddingCat(false)
  }
  ...
  async function moveCategory(id: string, direction: -1 | 1) {
    ...
    await Promise.all([
      supabase.from('product_categories').update({ position: b.position }).eq('id', a.id),
      supabase.from('product_categories').update({ position: a.position }).eq('id', b.id),
    ])
    await load(boutiqueId)
  }

  async function deleteCategory(id: string) {
    if (!boutiqueId || !confirm('Supprimer cette catégorie ? Les produits associés deviendront "Sans catégorie".')) return
            ...
            <div className="space-y-4">
              ...
              {plan === 'pro' ? (
                ...
                <p className="text-xs text-gray-400 border-t border-gray-100 pt-4">
                  Les variantes produit (taille, couleur, prix par option…) sont disponibles avec le plan Pro.
                </p>
              )}
              ...
              <div className="border-t border-gray-100 pt-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">Ce produit apparaîtra en tout premier sur la vitrine. 2 produits épinglés maximum.</p>
                  </div>
                  {canUseFeature(plan, 'epinglageProduits') && (
                    <input
                      type="checkbox"
                      id="is_pinned"
                ...
                {canUseFeature(plan, 'epinglageProduits') ? (
                  !form.is_pinned && otherPinnedCount >= 2 && (
                    <p className="text-xs text-amber-600 mt-2">Limite de 2 produits épinglés atteinte. Désépinglez-en un pour en épingler un autre.</p>
                  )
                ) : (
                  <p className="text-xs text-gray-400 mt-2">L&apos;épinglage de produits est disponible avec le plan Pro.</p>
                )}
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-[70] bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Ce produit n&apos;a plus de variantes. Confirmez le prix de base à utiliser désormais.
            </p>
            <label className="text-xs text-gray-400 mb-1 block">Prix (FCFA) *</label>
            <input
              type="number"
