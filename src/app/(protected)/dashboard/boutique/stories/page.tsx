...
function StoriesManager({ boutiqueId }: { boutiqueId: string }) {
  ...
  const load = useCallback(async (bid: string) => {
    const [{ data: storyRows }, { data: prods }] = await Promise.all([
      supabase.from('boutique_stories').select('*').eq('boutique_id', bid)
        .gt('expires_at', new Date().toISOString()).order('created_at', { ascending: false }),
      supabase.from('products').select('id, name, image_url').eq('boutique_id', bid).eq('is_available', true),
    ])
    setStories(storyRows ?? [])
    setProducts(prods ?? [])
    const { count } = await supabase.from('boutique_stories').select('id', { count: 'exact', head: true })
      .eq('boutique_id', bid).gte('created_at', startOfTodayISO())
    setTodayCount(count ?? 0)
  }, [supabase])

  useEffect(() => { load(boutiqueId) }, [load, boutiqueId])
  ...
  async function handleFileSelected(file: File) {
    ...
    const uploadedPath = await uploadWithProgress('boutique-stories', path, file, session.access_token, setUploadProgress)
    if (!uploadedPath) {
      setError('Erreur lors de l\'envoi du fichier. Vérifiez votre connexion et réessayez.')
      setUploading(false)
      return
    }
    const { data: pub } = supabase.storage.from('boutique-stories').getPublicUrl(uploadedPath)
    ...
    const { error: insertError } = await supabase.from('boutique_stories').insert({
  ...
  async function deleteStory(id: string) {
    if (!boutiqueId || !confirm('Supprimer cette story ?')) return
    await supabase.from('boutique_stories').delete().eq('id', id)
    await load(boutiqueId)
  }
  ...
  return (
    <div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-6 space-y-4">
        ...
        <div>
          <label className="text-xs text-gray-400 mb-1 block">Produit à mettre en avant (optionnel)</label>
          <select
            value={productId}
...
export default function StoriesPage() {
  ...
  useEffect(() => {
    fetch('/api/auth/me').then(r => r.json()).then(async data => {
      if (!data.boutique_id) return
      setBoutiqueId(data.boutique_id)
      const { data: sub } = await supabase.from('subscriptions').select('plan').eq('boutique_id', data.boutique_id).single()
      setPlan((sub?.plan as PlanKey) ?? 'starter')
    })
  }, [supabase])

  if (plan === null || !boutiqueId) return <p className="text-gray-400 text-sm py-8">Chargement...</p>
