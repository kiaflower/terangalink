...
function OrdersPageInner() {
    ...
    fetch('/api/auth/me').then(r => r.json()).then(async d => {
      if (d.boutique_id) {
        ...
        const { data: boutique } = await supabase
          ...
          .eq('id', d.boutique_id)
          .single()
        if (boutique) setBoutiqueInfo(boutique)
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => load(boutiqueId, true), 30000)
    return () => clearInterval(interval)
  }, [boutiqueId, load])

  function trackingUrl(order: Order): string {
    const origin = typeof window !== 'undefined' ? window.location.origin : 'https://teranga-spot.com'
    return `${origin}/c/${boutiqueInfo?.slug}/${order.order_number}`
  }
  ...
  async function updateStatus(orderId: string, status: string) {
    ...
    if (res.ok && boutiqueId) await load(boutiqueId)
  ...
  async function confirmOrder(order: Order) {
    ...
    if (!order.customer_phone || !boutiqueInfo) return
    ...
    const hasWave = !!boutiqueInfo.wave_number
    const hasOrangeMoney = !!boutiqueInfo.orange_money_number
    const showPaymentInfo = hasPreorder || (order.payment_method !== 'Cash' && (hasWave || hasOrangeMoney))
    const lines = [
      `Bonjour ${order.customer_name},`,
      hasPreorder
        ? `Votre précommande ${order.order_number} chez ${boutiqueInfo.name} a été confirmée !`
        : `Votre commande ${order.order_number} chez ${boutiqueInfo.name} a été confirmée !`,
    ]
    if (hasPreorder) {
    ...
    if (showPaymentInfo) {
      ...
      if (hasWave) lines.push(`Wave : ${boutiqueInfo.wave_number}`)
      if (hasOrangeMoney) lines.push(`Orange Money : ${boutiqueInfo.orange_money_number}`)
  ...
  function requestReview(order: Order) {
    ...
    const message = [
      ...
      `Nous espérons que vous êtes satisfait(e) de votre achat chez ${boutiqueInfo.name}.`,
          ...
          <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
            ...
            {boutiqueInfo && (
              <div className="flex flex-wrap gap-2 mb-4">
                <ReceiptGenerator order={selected} boutique={boutiqueInfo} plan={plan} />
              </div>
            )}
            ...
            <div className="border-t border-gray-200 py-4 mb-4">
              <h3 className="text-sm font-medium text-gray-400 mb-3">Produits</h3>
