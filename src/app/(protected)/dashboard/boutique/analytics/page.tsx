...
async function getBoutiqueId(userId: string, supabase: ReturnType<typeof createClient>): Promise<string | null> {
  const cookieStore = cookies()
  const imp = cookieStore.get('sa_impersonate')?.value
  if (imp) return imp
  const { data } = await supabase.from('profiles').select('boutique_id').eq('id', userId).single() as { data: { boutique_id: string | null } | null }
  return data?.boutique_id ?? null
}
...
export default async function AnalyticsPage() {
  ...
  const boutiqueId = await getBoutiqueId(user.id, supabase)
  if (!boutiqueId) return <div className="text-gray-500 py-20 text-center">Aucune boutique associée</div>

  const { data: sub } = await supabase.from('subscriptions').select('plan').eq('boutique_id', boutiqueId).single()
  ...
  const [views30Res, orders30Res] = await Promise.all([
    supabase.from('analytics_events').select('*', { count: 'exact', head: true }).eq('boutique_id', boutiqueId).eq('event_type', 'boutique_view').gte('created_at', d30Iso),
    supabase.from('orders').select('total, status').eq('boutique_id', boutiqueId).neq('status', 'cancelled').gte('created_at', d30Iso),
  ])
  ...
  return (
    <div>
      ...
      <FeatureGate plan={plan} feature="analyticsParcours" requiredPlanLabel="Starter"
        description="Débloquez le parcours d'achat (visites → commandes) et les produits les plus populaires.">
        <AdvancedAnalytics supabase={supabase} boutiqueId={boutiqueId} now={now} plan={plan} />
      </FeatureGate>
    </div>
  )
}

async function AdvancedAnalytics({ supabase, boutiqueId, now, plan }: {
  ...
  const [cartItemsRes, ordersItemsRes, weekStats] = await Promise.all([
    supabase.from('analytics_events').select('item_id, item_name').eq('boutique_id', boutiqueId).eq('event_type', 'add_to_cart').gte('created_at', d30Iso).limit(5000),
    supabase.from('orders').select('items').eq('boutique_id', boutiqueId).neq('status', 'cancelled').gte('created_at', d30Iso).limit(5000),
    computeWeeklyStats(supabase, boutiqueId, d7, now),
  ])

  // Un seul tableau "produits populaires" (30j), fusionnant ajouts panier et
  // commandes par nom de produit plutôt que deux listes séparées.
  ...
  const journeySteps = [
    ...
    { label: 'Vues produit', value: weekStats.productViews },
    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10 text-sm">
        <div className="bg-gray-50 rounded-xl p-3">
          <span className="text-gray-500">Vue produit → panier : </span>
      ...
      {popularProducts.length > 0 && (
        <div className={`${cardCls} mb-10`}>
          <SectionTitle title="Produits populaires (30 jours)" />
      ...
      <FeatureGate plan={plan} feature="analyticsAvances" requiredPlanLabel="Pro"
        description="Débloquez l'abandon de panier, les produits à surveiller et des recommandations personnalisées.">
