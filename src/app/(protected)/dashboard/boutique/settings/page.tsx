...
export default function SettingsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    if (!meRes.boutique_id) return
    setAdminRole(meRes.admin_role ?? null)
    setMyId(meRes.user_id ?? null)
    const { data: b } = await supabase.from('boutiques').select('id, name, slug, referral_code, whatsapp_number, opening_hours, newsletter_opt_in').eq('id', meRes.boutique_id).single()
    const { data: s } = await supabase.from('subscriptions').select('plan, status, ends_at, discount_percent, discount_expires_at').eq('boutique_id', meRes.boutique_id).single()
    if (b) {
      setBoutique(b)
  ...
  }, [boutique]) // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => { loadData() }, [loadData])
  useEffect(() => {
    if (boutique && isPrincipal && sub?.plan === 'pro') loadAdmins()
  }, [boutique, isPrincipal, sub?.plan]) // eslint-disable-line react-hooks/exhaustive-deps

  /* ─── Horaires ─── */
  async function saveHours() {
    if (!boutique) return
    setHoursSaving(true)
    await supabase.from('boutiques').update({ opening_hours: hours }).eq('id', boutique.id)
  ...
  async function toggleNewsletterOptIn(value: boolean) {
    ...
    await supabase.from('boutiques').update({ newsletter_opt_in: value }).eq('id', boutique.id)
  ...
  async function handlePasswordChange(e: React.FormEvent) {
    ...
    setPwSaving(true)
    const res = await fetch('/api/boutique/change-password', {
      method: 'POST',
  ...
  async function handleAddAdmin(e: React.FormEvent) {
    ...
    setAddAdminMsg(null)
    const res = await fetch('/api/boutique/admins/add', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function handleRemoveAdmin(id: string, label: string) {
    ...
    setRemovingId(id)
    const res = await fetch('/api/boutique/admins/remove', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function handleShare() {
    if (!boutique?.referral_code) return
    const url = `${window.location.origin}/inscription?ref=${boutique.referral_code}`
    if (navigator.share) {
      try { await navigator.share({ title: 'TerangaSpot', text: `Inscris ta boutique sur TerangaSpot avec mon code et on a tous les deux 25% de réduction !`, url }) }
      catch { /* cancelled */ }
    } else {
  ...
  async function handleExport() {
    ...
    const { data: products } = await supabase.from('products').select('name, price, is_available').eq('boutique_id', boutique.id)
    const { data: orders } = await supabase.from('orders').select(orderColumns).eq('boutique_id', boutique.id).gte('created_at', since).order('created_at', { ascending: false })

    const prodCsv = ['Produit,Prix,Disponible', ...(products ?? []).map(p => `"${p.name}",${p.price},${p.is_available ? 'Oui' : 'Non'}`)].join('\n')
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `${boutique.slug}-export.csv`; a.click()
    setExporting(false)
  }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      {/* ── Section 1 : Mot de passe (partagé par la boutique) — principal uniquement ── */}
      {isPrincipal && (
      ...
      <NotificationSettings variant="boutique" />
        ...
        {boutique?.referral_code ? (
          <div>
            ...
            <p className="text-xs text-gray-400">
              {siteOrigin.replace(/^https?:\/\//, '')}/inscription?ref={boutique.referral_code}
            </p>
          </div>
        ) : (
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6">
        ...
        <p className="text-xs text-gray-500 mb-4">
          Exportez votre catalogue et vos commandes au format CSV.{' '}
          {sub?.plan === 'pro'
        ...
        <button onClick={handleExport} disabled={exporting || !boutique}
