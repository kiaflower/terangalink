...
export default async function BoutiqueDashboardHome() {
  ...
  if (!boutiqueId) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-gray-700 font-medium mb-1">Aucune boutique associée</p>
        <p className="text-sm text-gray-400">Contactez le support TerangaSpot.</p>
      </div>
    )
  }
  ...
  const [
    ...
    supabase.from('orders').select('total, status').eq('boutique_id', boutiqueId).gte('created_at', today.toISOString()),
    supabase.from('orders').select('total').eq('boutique_id', boutiqueId).eq('status', 'delivered').gte('created_at', monthStart),
    supabase.from('products').select('id', { count: 'exact', head: true }).eq('boutique_id', boutiqueId).eq('is_available', true),
    supabase.from('analytics_events').select('id', { count: 'exact', head: true }).eq('boutique_id', boutiqueId).eq('event_type', 'boutique_view').gte('created_at', new Date(Date.now() - 7 * 86400000).toISOString()),
    supabase.from('orders').select('*').eq('boutique_id', boutiqueId).order('created_at', { ascending: false }).limit(5),
    supabase.from('products').select('id, name, image_url, stock_quantity').eq('boutique_id', boutiqueId).eq('track_stock', true).eq('is_available', true).lte('stock_quantity', 5).order('stock_quantity'),
  ])
    ...
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">
          {boutique ? `Bienvenue, ${boutique.name}` : 'Tableau de bord'}
        </h1>
      ...
      {boutique && (
        <div className="bg-gray-50 border border-brand-orange/10 rounded-2xl p-5 mb-6 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-gray-900 font-semibold">{boutique.name}</p>
            <p className="text-gray-500 text-sm">{boutique.city || 'Dakar'}</p>
          </div>
          {subscription && (
      ...
      <div className="mb-6">
        <FeatureGate plan={plan} feature="qrCode" requiredPlanLabel="Starter"
          description="Débloquez le QR code téléchargeable, les avis clients, le suivi de commande, les bannières, jusqu'à 30 produits et le parcours d'achat dans vos analytiques.">
          {null}
        </FeatureGate>
        <FeatureGate plan={plan} feature="analyticsAvances" requiredPlanLabel="Pro"
          description="Débloquez le catalogue illimité, les variantes produit, la gestion de stock, les codes promo, les précommandes et les analytiques complètes (abandon de panier, recommandations).">
          {null}
        </FeatureGate>
      </div>
      ...
      <div className="stats-grid mb-8">
        ...
        <StatCard
          ...
          color="blue"
          subtitle={(productsCount ?? 0) === 0 ? 'Aucun produit actif' : `${productsCount} produit(s) disponible(s)`}
        />
        <StatCard
          title="Vues (7 jours)"
      ...
      <Card className="mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            { label: 'Gérer le catalogue', desc: 'Ajouter et modifier vos produits', href: '/dashboard/boutique/catalogue', icon: Package },
            { label: 'Voir les commandes', desc: 'Commandes en cours et historique', href: '/dashboard/boutique/orders', icon: ShoppingBag },
            { label: 'Profil de la boutique', desc: 'Modifier les informations', href: '/dashboard/boutique/profile', icon: TrendingUp },
