...
export default function ParrainagePage() {
  ...
  const [referred, setReferred] = useState<ReferredBoutique[]>([])
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: boutique }, { count }, { data: sub }] = await Promise.all([
      supabase.from('boutiques').select('referral_code').eq('id', bId).single(),
      supabase.from('referral_credits').select('id', { count: 'exact', head: true }).eq('boutique_id', bId).eq('status', 'available'),
      supabase.from('subscriptions').select('pending_credit_action').eq('boutique_id', bId).maybeSingle(),
    ])

    if (boutique?.referral_code) {
      setReferralCode(boutique.referral_code)
      const { data: referredData } = await supabase
        .from('boutiques')
        .select('slug, name, created_at, referral_rewards!referred_boutique_id(status, triggered_at)')
        .eq('referred_by_code', boutique.referral_code)
        .order('created_at', { ascending: false })
      setReferred((referredData ?? []) as unknown as ReferredBoutique[])
    }
  ...
  async function runAction(kind: 'discount' | 'free_month') {
    ...
    const endpoint = kind === 'discount'
      ? '/api/boutique/referral-credits/apply-discount'
      : '/api/boutique/referral-credits/redeem-free-month'
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">boutique{referred.length !== 1 ? 's' : ''} parrainée{referred.length !== 1 ? 's' : ''}</p>
            </div>
          </div>

          <div className="bg-white border border-gray-200 rounded-2xl p-6">
            <p className="font-bold text-gray-900 mb-4">Boutiques parrainées</p>
            {referred.length === 0 ? (
              <p className="text-sm text-gray-400">Vous n&apos;avez pas encore parrainé de boutique.</p>
            ) : (
