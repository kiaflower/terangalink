...
function BannersManager({ restaurantId }: { restaurantId: string }) {
  ...
  async function addBanner() {
    ...
    setSaving(true)
    setError(null)
    const { error } = await supabase.from('restaurant_banners').insert({ restaurant_id: restaurantId, text: newText.trim(), is_active: true })
  ...
  async function toggleBanner(banner: Banner) {
    ...
    setError(null)
    await supabase.from('restaurant_banners').update({ is_active: !banner.is_active }).eq('id', banner.id)
    await load()
  }

  async function deleteBanner(id: string) {
    await supabase.from('restaurant_banners').delete().eq('id', id)
    await load()
  }
...
export default function BannersPage() {
  ...
  useEffect(() => {
    fetch('/api/auth/me').then(r => r.json()).then(async d => {
      if (!d.restaurant_id) { setLoading(false); return }
  ...
  return (
    <FeatureGate plan={plan} feature="bannieres" requiredPlanLabel="Starter"
      description="Affichez des bannières promotionnelles au-dessus de votre menu pour mettre en avant vos offres et annonces.">
      <BannersManager restaurantId={restaurantId} />
    </FeatureGate>
  )
}
