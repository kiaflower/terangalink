...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: targetData } = await admin
      .from('profiles')
      .select('id, boutique_id, role, admin_role')
      .eq('id', admin_id)
      .single()
    const target = targetData as { id: string; boutique_id: string | null; role: string; admin_role: string | null } | null

    if (!target || target.boutique_id !== profile.boutique_id || target.role !== 'boutique_admin') {
    ...
    console.error('boutique admins/remove error:', err)
