...
import { rotateBoutiquePassword } from '@/lib/auth/boutiqueAdmin'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // (utilisé pour les autres admins dans rotateBoutiquePassword), un update
    ...
    const { data: boutiqueData } = await admin.from('boutiques').select('name').eq('id', profile.boutique_id).single()
    const boutiqueName = (boutiqueData as { name: string } | null)?.name ?? 'votre boutique'

    const { admins } = await rotateBoutiquePassword(admin, profile.boutique_id, next, user.id)

    const others = admins.filter(a => a.id !== user.id)
    await Promise.all(others.map(a => sendMail({ to: a.email, ...passwordChangedEmail(boutiqueName, a.email, next) })))
