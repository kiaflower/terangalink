...
/**
 * Prépare le rapport hebdomadaire d'un restaurant (parcours d'achat, abandon
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: restaurant } = await admin.from('restaurants').select('id, name, whatsapp_number').eq('id', restaurantId).single()
    if (!restaurant) return NextResponse.json({ error: 'Restaurant introuvable' }, { status: 404 })
    if (!restaurant.whatsapp_number) return NextResponse.json({ error: 'Aucun numéro WhatsApp enregistré pour ce restaurant' }, { status: 400 })

    const periodEnd = new Date()
    const periodStart = new Date(periodEnd.getTime() - 7 * 24 * 60 * 60 * 1000)

    const stats = await computeWeeklyStats(admin, restaurantId, periodStart, periodEnd)
    const recommendations = computeRecommendations(stats)
    const message = buildWeeklyReportMessage(restaurant.name, stats, recommendations.map(r => r.message))
