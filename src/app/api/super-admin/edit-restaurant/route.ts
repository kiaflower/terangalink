...
export async function POST(request: NextRequest) {
  try {
    ...
    const { id, name, slug, city, cuisine_type, description, whatsapp_number, phone,
    ...
    if (typeof cuisine_type === 'string') payload.cuisine_type = cuisine_type || null
    ...
    const { data: sub } = await supabase.from('subscriptions').select('plan').eq('restaurant_id', id).single()
