...
// réduction au moment de l'encaissement si le restaurant n'en a pas déjà
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { restaurant_id, method, apply_credit } = await request.json()
    ...
    const result = await collectPayment(admin, { restaurantId: restaurant_id, method: paymentMethod, applyCredit })
