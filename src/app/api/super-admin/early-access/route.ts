...
export async function POST(req: NextRequest) {
  ...
  // Idempotent — clicking "Confirmer" twice must not create a second restaurant.
  if (application.created_restaurant_id) {
    return NextResponse.json({ ok: true, restaurant_id: application.created_restaurant_id, already_created: true })
  }

  // Defensive fallback: if a previous attempt created the restaurant/account but failed
  // to persist created_restaurant_id (e.g. the tracking update below errored), reuse it
  // instead of creating a duplicate restaurant + a duplicate auth account for the same email.
  const { data: existingProfile } = await admin.from('profiles').select('restaurant_id').eq('email', application.email).maybeSingle()
  if (existingProfile?.restaurant_id) {
    await admin.from('early_access_applications').update({
      status: 'confirmed',
      created_restaurant_id: existingProfile.restaurant_id,
    }).eq('id', id)
    return NextResponse.json({ ok: true, restaurant_id: existingProfile.restaurant_id, already_created: true })
  }

  const slug = slugify(application.restaurant_name || 'restaurant')
  ...
  const { data: restaurant, error: restaurantError } = await admin.from('restaurants').insert({
    ...
    cuisine_type: application.cuisine_type || null,
  ...
  const { data: newUser, error: userError } = await admin.auth.admin.createUser({
    ...
    email_confirm: true,
    user_metadata: { full_name: application.owner_name, role: 'restaurant_admin' },
  })
  ...
  if (trackingError) {
    ...
    return NextResponse.json({
      error: `Restaurant créée, mais impossible d'enregistrer la confirmation (${trackingError.message}). Vérifiez que la migration 0009 a bien été exécutée sur Supabase.`,
      restaurant_id: restaurant.id,
    }, { status: 500 })
  }

  await sendMail({ to: application.email, ...welcomeEmail(restaurant.name, application.email, adminPassword) })
