...
const SAMPLE_PRODUCTS = [
  { name: 'Plat exemple', price: '10 000 FCFA' },
  { name: 'Autre plat', price: '7 500 FCFA' },
]

export function RestaurantLivePreview({ name, category, city, logoUrl, coverUrl, primaryColor, theme }: Props) {
  const { accent, pageBg, pageText, cardBg, cardBorder, subtleText } = getRestaurantTheme({ primary_color: primaryColor, theme })

  return (
    <div className="rounded-2xl overflow-hidden border border-gray-200 text-sm" style={{ backgroundColor: pageBg }}>
      <p className="text-[10px] text-gray-400 px-4 pt-3 pb-1 font-medium uppercase tracking-wider bg-gray-50">Prévisualisation de votre restaurant</p>
      ...
      <div className="flex items-center gap-2 px-4 py-3" style={{ borderBottom: `1px solid ${cardBorder}` }}>
        ...
        <div className="min-w-0">
          <p className="font-bold text-xs truncate" style={{ color: pageText }}>{name || 'Nom de votre restaurant'}</p>
      ...
      <div className="relative h-28">
        ...
        <div className="absolute bottom-2.5 left-3 right-3">
          ...
          <p className="font-black text-white text-lg leading-tight truncate">{name || 'Nom de votre restaurant'}</p>
