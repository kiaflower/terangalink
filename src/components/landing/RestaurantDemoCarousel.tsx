...
import { RestaurantPhoneDemo } from './RestaurantPhoneDemo'
...
export function RestaurantDemoCarousel({ entries }: { entries: DemoEntry[] }) {
  ...
  return (
    <div className="relative w-full max-w-xs mx-auto select-none">
      ...
      >
        {entries.map(entry => (
          <div key={entry.restaurant.id} className="flex-shrink-0 w-full snap-center flex justify-center">
            <RestaurantPhoneDemo
      ...
      {entries.length > 1 && (
        <>
          <button type="button" onClick={() => scrollToIndex(activeIndex - 1)} disabled={activeIndex === 0}
            aria-label="Restaurant précédente"
          ...
          <button type="button" onClick={() => scrollToIndex(activeIndex + 1)} disabled={activeIndex === entries.length - 1}
            aria-label="Restaurant suivante"
          ...
          <div className="flex items-center justify-center gap-2 mt-5">
            {entries.map((entry, i) => (
              <button key={entry.restaurant.id} type="button" onClick={() => scrollToIndex(i)}
                aria-label={`Voir le restaurant ${i + 1}`}
