...
import RestaurantPageClient from '@/app/[slug]/RestaurantPageClient'
...
interface RestaurantPhoneDemoProps {
...
// ResizeObserver — pas d'iframe, donc les modals `position: fixed` de RestaurantPageClient
...
export function RestaurantPhoneDemo({ restaurant, categories, products, reviews = [], banners = [], plan, label }: RestaurantPhoneDemoProps) {
  const accentColor = restaurant.primary_color || '#F97316'
  const planLabel = label ?? (plan === 'pro' ? 'Exemple de restaurant Pro' : 'Exemple de restaurant Starter')
  ...
  // Boucle d'animation : pilote le vrai DOM de RestaurantPageClient (getBoundingClientRect / click réels)
  useEffect(() => {
    let cancelled = false
    ...
    async function run() {
      ...
      while (!cancelled) {
        ...
        // Ajouter un plat au panier (carte plat → modal → "Ajouter au panier")
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <div className="relative mx-auto aspect-[9/19] w-[220px] overflow-hidden rounded-[2.5rem] border-8 border-gray-900 bg-white shadow-2xl">
        {/* RestaurantPageClient utilise les breakpoints sm:/md:/lg: de Tailwind, qui réagissent à la
        ...
        <div ref={frameRef} className="tsp-demo-frame relative h-full w-full overflow-hidden">
          <div className="absolute left-0 top-0 origin-top-left"
            ...
            <div ref={scrollRef} className="h-full w-full overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              <RestaurantPageClient
                ...
                banners={banners}
                previewMode
                demoHomeHref="/pour-les-restaurants"
              />
            </div>
          </div>
          ...
          {/* Un vrai clic (utilisateur) ouvre la vraie vitrine dans un nouvel onglet plutôt que de
              ...
              démo ("← Accueil" / "Créer mon restaurant →") en dehors de cette zone pour que ses liens
              fonctionnent normalement au clic. */}
          <div
            className="absolute left-0 right-0 bottom-0 z-40 cursor-pointer"
            style={{ top: restaurant.is_demo ? 40 * fit.scale : 0 }}
            onClick={() => window.open(`/${restaurant.slug}`, '_blank', 'noopener,noreferrer')}
            role="link" tabIndex={0}
            aria-label={`Voir le restaurant démo ${restaurant.name}`}
            onKeyDown={e => {
              if (e.key === 'Enter') window.open(`/${restaurant.slug}`, '_blank', 'noopener,noreferrer')
            }}
          />
