  ...
  if (status === 'low') {
    return (
      <div className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full"
        ...
        Stock faible — Plus que {qty} disponible{qty > 1 ? 's' : ''}
      </div>
    )
  }
...
function PreorderBanner({ item, tokens }: { item: MenuItem; tokens: ThemeTokens }) {
  ...
  const closeCountdown = useCountdown(
    preorderState?.status === 'open' ? preorderState.closeAt : null
  )
  const openCountdown = useCountdown(
    preorderState?.status === 'soon' ? preorderState.openAt : null
  )

  if (!preorderState) return null

  if (preorderState.status === 'soon') {
    return (
      <div className="rounded-xl p-3 text-xs font-medium"
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="font-semibold">Précommandes bientôt ouvertes</span>
        </div>
        <p>Ouverture dans : <span className="font-bold">{openCountdown}</span></p>
        {item.preorder_delivery_date && (
          <p className="mt-0.5 opacity-75">
            Livraison : {item.preorder_delivery_date}
          </p>
        )}
      </div>
    )
          ...
          <div className="mb-1.5">
            <div className="flex items-center justify-between mb-1 opacity-75">
              ...
              {item.preorder_max_qty && (
                <span>{item.preorder_reserved ?? 0} / {item.preorder_max_qty} réservés</span>
              )}
            </div>
            {item.preorder_max_qty && (
              <div className="h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: 'rgba(255,255,255,0.12)' }}>
                <div
                  ...
                  style={{
                    width: `${Math.min(100, ((item.preorder_reserved ?? 0) / item.preorder_max_qty) * 100)}%`,
  ...
  return (
    <div className="rounded-xl p-3 text-xs font-medium"
      ...
      <div className="flex items-center gap-2">
        <Clock className="w-3.5 h-3.5" />
        <span className="font-semibold">Précommandes terminées</span>
      </div>
    </div>
  )
}
...
export function ProductModal({
  ...
  const handleAdd = useCallback(() => {
    ...
    if (hasVariants && !selectedVariant) return // selection obligatoire

    const effectivePrice = selectedVariant ? selectedVariant.price : item.price
    const effectiveName = item.name
    const cartId = selectedVariant ? `${item.id}__${selectedVariant.id}` : item.id
      ...
      addItem(
        {
          ...
          preorder_delivery_date: (isPremium && item.preorder_enabled) ? (item.preorder_delivery_date ?? null) : null,
        },
        restaurantId,
        restaurantSlug,
        restaurantPhone,
    ...
    <div className="fixed inset-0 z-[90] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      >
        ...
        <div className="overflow-y-auto flex-1">
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="flex items-start justify-between gap-3">
              <h2 className="text-xl font-bold leading-tight" style={{ color: tokens.textPrimary }}>
                {item.name}
              </h2>
              <div className="text-right flex-shrink-0">
                {priceLabel && (
                  <p className="text-xs font-medium mb-0.5" style={{ color: tokens.textMuted }}>{priceLabel}</p>
                )}
                <p className="text-xl font-black" style={{ color: tokens.accentText }}>
        ...
        <div
          ...
          style={{ borderTop: `1px solid ${tokens.border}`, backgroundColor: tokens.bgCard }}
        >
          {/* Quantité */}
          ...
          {!item.is_available ? (
            ...
            <button
              onClick={handleAdd}
              disabled={isBlockedByVariant || added}
            ...
            >
              ...
              {added
                ...
                : isBlockedByVariant
                  ? 'Choisir une option'
                  : `Ajouter — ${formatCurrency(displayPrice * qty)}`
              }
            </button>
          )}
        </div>
