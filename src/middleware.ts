...
export async function middleware(request: NextRequest) {
  ...
  if (isLoginPage) {
    ...
    if (role === 'boutique_admin') {
      ...
      // on redirigeait quand même vers /dashboard/boutique, son propre
      ...
      const sessionCheck = await checkBoutiqueSession(supabase, user.id)
      if (!sessionCheck.ok) {
        await supabase.auth.signOut()
        return supabaseResponse
      }
      return NextResponse.redirect(new URL('/dashboard/boutique', request.url))
    }
    if (role) return NextResponse.redirect(new URL('/dashboard/boutique', request.url))
    return supabaseResponse
  }
  ...
  const profile = profileData as { role: string; boutique_id: string | null } | null
  ...
  // Super admin with impersonation cookie → allow boutique dashboard.
  ...
  if (isSuperAdmin && impersonateCookie && pathname.startsWith('/dashboard/boutique')) {
    return supabaseResponse
  }
  ...
  if (!isSuperAdmin && pathname.startsWith('/dashboard/boutique')) {
    const sessionCheck = await checkBoutiqueSession(supabase, user.id)
    if (!sessionCheck.ok) {
  ...
  // Boutique admin → redirect away from super-admin
