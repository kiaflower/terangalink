...
export interface BoutiqueAdminProfile {
  id: string
  email: string
...
export async function getActiveAdmins(admin: SupabaseClient, boutiqueId: string): Promise<BoutiqueAdminProfile[]> {
  ...
  return (data ?? []) as BoutiqueAdminProfile[]
}

export async function getPrincipalAdmin(admin: SupabaseClient, boutiqueId: string): Promise<BoutiqueAdminProfile | null> {
  const { data } = await admin
    .from('profiles')
    .select('id, email, full_name, admin_role')
    .eq('boutique_id', boutiqueId)
    ...
    .eq('is_active', true)
    .maybeSingle()
  return (data ?? null) as BoutiqueAdminProfile | null
}

/** Déchiffre le mot de passe partagé actuel, ou null s'il n'a jamais été défini. */
export async function getCurrentPassword(admin: SupabaseClient, boutiqueId: string): Promise<string | null> {
  const { data } = await admin.from('boutiques').select('admin_password_enc').eq('id', boutiqueId).single()
...
interface RotateResult {
  newVersion: number
  admins: BoutiqueAdminProfile[]
}

/**
 * Pousse newPassword sur tous les comptes Supabase Auth des admins actifs de
 * la boutique, incrémente boutiques.password_version, chiffre et stocke le
 ...
 * mots de passe Supabase changés sans que boutiques.password_version /
 * admin_password_enc ne soient mis à jour en conséquence.
 */
export async function rotateBoutiquePassword(
  ...
  const admins = await getActiveAdmins(admin, boutiqueId)
  ...
  results.forEach((r, i) => {
    if (r.error) console.error(`[rotateBoutiquePassword] échec rotation pour ${othersToRotate[i].email}:`, r.error)
  })

  const { data: boutiqueData } = await admin.from('boutiques').select('password_version').eq('id', boutiqueId).single()
  const currentVersion = (boutiqueData as { password_version: number } | null)?.password_version ?? 1
  const newVersion = currentVersion + 1

  const { error: updateError } = await admin.from('boutiques').update({
    password_version: newVersion,
    admin_password_enc: encryptedPassword,
  }).eq('id', boutiqueId)
  if (updateError) throw new Error(`Échec mise à jour boutiques.password_version: ${updateError.message}`)
